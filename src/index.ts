...
class Intune {
    ...
    async autopilotUpload({
        serialNumber,
        groupTag,
        productKey,
        hardwareIdentifier,
        assignedUser,
    }: AutoPilotUpload): Promise<object> {
        try {
            const postBody = {
    ...
    async getUserAppsIntentandState(userId: string): Promise<object[]> {
        try {
            const res = await this._IntuneRequest(
                `${this.domain}/users/${userId}/mobileAppIntentAndStates`,
                {
                    method: 'GET',
                    headers: this.reqHeaders,
                },
    ...
    async getUserAppIntentandState(userId: string, appIntentAndStateId: string): Promise<object[]> {
        try {
            const res = await this._IntuneRequest(
                `${this.domain}/users/${userId}/mobileAppIntentAndStates/${appIntentAndStateId}`,
                {
                    method: 'GET',
                    headers: this.reqHeaders,
                },
    ...
    async getGroupMembers(groupId: string): Promise<object[]> {
        try {
            const res = await this._IntuneRequest(
                `${this.domain}/groups/${groupId}/members?$top=999`,
                {
                    method: 'GET',
                    headers: this.reqHeaders,
                },
    ...
    async getGroupOwners(groupId: string): Promise<object[]> {
        try {
            const res = await this._IntuneRequest(
                `${this.domain}/groups/${groupId}/owners?$top=999`,
                {
                    method: 'GET',
                    headers: this.reqHeaders,
                },
    ...
    async removeGroupMember(groupId: string, userId: string): Promise<object> {
        try {
            const res = await this._IntuneRequest(
                `${this.domain}/groups/${groupId}/members/${userId}/$ref`,
                {
                    method: 'DELETE',
                    headers: this.reqHeaders,
                },
    ...
    async removeGroupOwner(groupId: string, userId: string): Promise<object> {
        try {
            const res = await this._IntuneRequest(
                `${this.domain}/groups/${groupId}/owners/${userId}/$ref`,
                {
                    method: 'DELETE',
                    headers: this.reqHeaders,
                },
    ...
    async createApp(postBody: object): Promise<object> {
        try {
            const res = await this._IntuneRequest(`${this.domain}/deviceAppManagement/mobileApps`, {
                method: 'POST',
                headers: this.reqHeaders,
                body: JSON.stringify(postBody),
            })
    ...
    async createWin32app(
        ...
        try {
            ...
            const azureStorageUri = await this.getAzureStorageUri(appId, contentVersionId, fileId)
            const fileSize = fileInfoBody.size

            await this.uploadToAzureBlob(azureStorageUri, file, fileSize)

            await this.commitFileUpload(appId, contentVersionId, fileId, encryptionBody)
